...
**Key Characteristics:**
- Monochrome system core: `{colors.primary}` (black) and `{colors.canvas}` (white) carry the default CTA pair, every body line, every footer link; **`{colors.accent-sakura}`** (sakura pink) is the only branded chromatic accent, used sparingly for standout promo pills.
...
The palette is deliberately **three-part**: black and white anchor the chrome, typography, and CTAs; **sakura pink** is the only brand chromatic accent — soft cherry-blossom pink used for focal moments, not as a recurring section fill.
...
- **Sakura pink** ({colors.accent-sakura}): The brand accent — a soft sakura/cherry-blossom pink reserved for standout CTAs (e.g., promotional inline buttons on banners). Use sparingly; it is not a large section background color except where a single panel needs unmistakable warmth.
...
**`button-sakura-promo`** — Sakura pink pill used only inside promotional surfaces such as the lilac "Save your spot" Release Notes banner. Reserved for moments where the CTA should pop against an already-colored panel while staying on-brand with the black / white / sakura trio.
- Background `{colors.accent-sakura}`, text `{colors.primary}` (black on the light blossom tone), type `{typography.button}`, rounded `{rounded.pill}`, padding 10px 18px.
...
- Background `{colors.block-lilac}`, text `{colors.ink}`, type `{typography.body-sm}`, rounded `{rounded.md}`, padding `{spacing.md}` `{spacing.lg}`. Carries a `button-sakura-promo` on the right edge.
...
- Don't introduce new accent colors outside the documented `{colors.block-*}` palette and `{colors.accent-sakura}`. Adding, e.g., a saturated brand orange would break the system.
...
7. Treat `{colors.accent-sakura}` as a single-shot color: one promo CTA per page, never two.
